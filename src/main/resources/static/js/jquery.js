...
(function(window, undefined) {
	...
	var jQuery = (function() {
					...
					if (match && (match[1] || !context)) {
						...
						if (match[1]) {
							...
							if (ret) {
								...
								selector = (ret.cacheable ? jQuery.clone(ret.fragment) : ret
									.fragment).childNodes;
							}
		...
		jQuery.extend({
			...
			ready: function(wait) {
				...
				if ((wait === true && !--jQuery.readyWait) || (wait !== true && !jQuery
					.isReady)) {
			...
			map: function(elems, callback, arg) {
				var value, key, ret = [],
					...
					typeof length === "number" && ((length > 0 && elems[0] && elems[length -
						1]) || length === 0 || jQuery.isArray(elems));
		...
		jQuery.each("Boolean Number String Function Array Date RegExp Object".split(" "), function(i,
		name) {
								...
								if (jQuery.isFunction(fn)) {
									deferred[handler](function() {
										...
										if (returned && jQuery.isFunction(
												returned.promise)) {
											returned.promise().then(newDefer
												.resolve, newDefer
												.reject, newDefer.notify
												);
										} else {
			...
			select: {
				get: function(elem) {
					...
					for (; i < max; i++) {
						...
						if (option.selected && (jQuery.support.optDisabled ? !option.disabled : option
	...
	jQuery.event = {
		...
		add: function(elem, types, handler, data, selector) {
			...
			if (elem.nodeType === 3 || elem.nodeType === 8 || !types || !handler || !(elemData = jQuery
					._data(elem))) {
				return;
			}
		...
		trigger: function(event, data, elem, onlyHandlers) {
			...
			if (!onlyHandlers && !event.isDefaultPrevented()) {
				...
				if ((!special._default || special._default.apply(elem.ownerDocument, data) === false) &&
					...
					if (ontype && elem[type] && ((type !== "focus" && type !== "blur") || event.target
							.offsetWidth !== 0) && !jQuery.isWindow(elem)) {
		...
		dispatch: function(event) {
			...
			for (i = 0; i < handlerQueue.length && !event.isPropagationStopped(); i++) {
				...
				for (j = 0; j < matched.matches.length && !event.isImmediatePropagationStopped(); j++) {
					...
					if (run_all || (!event.namespace && !handleObj.namespace) || event.namespace_re && event
						.namespace_re.test(handleObj.namespace)) {
			...
			if (toString.call(checkSet) === "[object Array]") {
				if (!prune) {
					...
					for (i = 0; checkSet[i] != null; i++) {
						if (checkSet[i] && (checkSet[i] === true || checkSet[i].nodeType === 1 && Sizzle
		...
		var Expr = Sizzle.selectors = {
			...
			filters: {
				...
				text: function(elem) {
					...
					return elem.nodeName.toLowerCase() === "input" && "text" === type && (attr ===
						type || attr === null);
				},
			...
			filter: {
				...
				TAG: function(elem, match) {
					return (match === "*" && elem.nodeType === 1) || !!elem.nodeName && elem.nodeName
						.toLowerCase() === match;
				},
				...
				Expr.find.ID = function(match, context, isXML) {
					if (typeof context.getElementById !== "undefined" && !isXML) {
						...
						return m ?
							...
							.getAttributeNode("id").nodeValue === match[1] ? [m] :
							undefined : [];
					}
				};
					...
					jQuery.each(scripts, function(i, elem) {
						if (elem.src) {
							...
							jQuery.globalEval((elem.text || elem.textContent || elem
								.innerHTML || "").replace(rcleanScript, "/*$0*/"));
						}
	...
	jQuery.extend({
		...
		clean: function(elems, context, fragment, scripts) {
			...
			if (fragment) {
				...
				for (i = 0; ret[i]; i++) {
					script = ret[i];
					if (scripts && jQuery.nodeName(script, "script") && (!script.type || rscriptType
							.test(script.type))) {
						scripts.push(script.parentNode ? script.parentNode.removeChild(script) :
						script);
	...
	jQuery.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
		...
		var inspectData = (typeof s.data === "string") && /^application\/x\-www\-form\-urlencoded/.test(s
			.contentType);
	...
	jQuery.ajaxTransport("script", function(s) {
		...
		if (s.crossDomain) {
			...
			var script,
				head = document.head || document.getElementsByTagName("head")[0] || document
				.documentElement;
	...
	jQuery.fn.extend({
			...
			function doAnimation() {
				...
				for (name in prop) {
					...
					if (isElement && (name === "height" || name === "width")) {
						...
						opt.overflow = [this.style.overflow, this.style.overflowX, this.style
						.overflowY];
						...
						if (jQuery.css(this, "display") === "inline" &&
							...
							if (!jQuery.support.inlineBlockNeedsLayout || defaultDisplay(this
								.nodeName) === "inline") {
		...
		stop: function(type, clearQueue, gotoEnd) {
			...
			return this.each(function() {
				...
				if (type == null) {
					for (index in data) {
						if (data[index] && data[index].stop && index.indexOf(".run") === index
							.length - 4) {
	...
	if ("getBoundingClientRect" in document.documentElement) {
		getOffset = function(elem, doc, docElem, box) {
			...
			var body = doc.body,
				...
				scrollLeft = win.pageXOffset || jQuery.support.boxModel && docElem.scrollLeft || body
				.scrollLeft,
		...
		getOffset = function(elem, doc, docElem) {
			...
			while ((elem = elem.parentNode) && elem !== body && elem !== docElem) {
				...
				if (jQuery.support.subtractsBorderForOverflowNotVisible && computedStyle.overflow !==
					"visible") {
					top += parseFloat(computedStyle.borderTopWidth) || 0;
					left += parseFloat(computedStyle.borderLeftWidth) || 0;
				}
